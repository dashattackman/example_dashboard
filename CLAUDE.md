...
- **Cloud-session asset channel (probed, works):** raw.githubusercontent.com, media.githubusercontent.com (LFS), and `git clone` of public GitHub repos — direct asset-site downloads (quaternius.com/kenney.nl/polyhaven/ambientcg) are blocked. Fetch CC0 assets via GitHub mirrors; document provenance in CREDITS.md. Rig-scale gotcha: see `game/assets-pipeline/README` (bind pose at 1/100 scale — bake outlines into the glb, never shader-push).
