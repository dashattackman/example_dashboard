...
- **Device tiers** (auto-detected at boot from GPU string + a 2s hidden benchmark; user-overridable in settings). The budget table above is the **BASE tier floor** — mid-range must always play great. Flagships step UP, not the game down:
  ...
  - **WebGPU**: **auto-default where available**, WebGL2 universal fallback, `?webgl2` forces the fallback for A/B checks. (History: demoted to opt-in at M1 after an untested WGSL path blue-screened phones; restored 2026-08-01 after the flip-back criterion passed — clean run on Paul's device plus a real WebGPU transport pass in CI. Lesson kept: every new shader/material must pass the webgpu e2e project before merge.)
