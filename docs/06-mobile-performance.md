...
- **Device tiers** (auto-detected at boot from GPU string + a 2s hidden benchmark; user-overridable in settings). The budget table above is the **BASE tier floor** — mid-range must always play great. Flagships step UP, not the game down:
  ...
  - **WebGPU**: Babylon's WebGPU engine is used automatically where available (modern Chrome/Safari), WebGL2 as the universal fallback. Same scene code — Babylon abstracts it — but validate both paths in e2e from M1 on, and treat WebGPU as the expected HIGH-tier transport.
